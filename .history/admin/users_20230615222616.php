		...
		<div class="card-body">
			<div class="table-responsive">
				...
				<table class="table border-primary table-hover" id="dataTable" width="100%" cellspacing="0">
					...
					<tbody>
						<?php
						$get_customers = $cs->showsAllCustomer();
						if ($get_customers) {
							$i = 0;
							foreach ($get_customers as $result) {
								$i++;
								?>
								<tr class="odd gradeX">
									<td>
									...
									<td style="display: flex; flex-wrap: nowrap; gap: 4px">
										<button type="button" class="btn btn-primary">
											<a class="text-white"
												href="userEdit.php?customerid=<?php echo $result['customer_id'] ?>">Xem</a>
										</button>
										<button type="button" class="btn btn-danger">
											<a class="text-white" onclick="return confirm('Bạn chắc chắn xóa chứ ?')"
												href="?delid=<?php echo $result['customer_id'] ?>">Xóa</a>
										</button>
									</td>
								</tr>
								<?php
