...
<div class="bg-main">
    ...
    <?php
            $breadcrumbs = [
                ['param' => 'keyword', 'label' => 'keyword', 'link' => $_POST['keyword'] ?? ''],
                ['param' => 'catid', 'label' => 'catName', 'data' => 'show_category', 'link' => $_GET['catid'] ?? ''],
                ['param' => 'brandid', 'label' => 'brandName', 'data' => 'show_brand', 'link' => $_GET['brandid'] ?? ''],
                ['param' => 'isStock', 'label' => 'Stock', 'link' => '', 'condition' => isset($_GET['isStock'])]
            ];

            foreach ($breadcrumbs as $item) {
                if (isset($_REQUEST[$item['param']]) || (isset($item['condition']) && $item['condition'])) {
                    echo '<span><i class="bx bxs-chevrons-right"></i></span>';
                    echo '<a href="' . ($item['link'] ? '?' . $item['param'] . '=' . $item['link'] : '') . '">';
                    if (isset($item['data'])) {
                        $data = $item['data'];
                        $show_data = $data();
                        foreach ($show_data as $result) {
                            if ($result[$item['param']] == $item['link']) {
                                echo $result[$item['label']];
                                break;
                            }
                        }
                    } else {
            ...
            <span><i class='bx bxs-chevrons-right'></i></span>
            <a href="">
                <?php echo $total_results ?> sản phẩm
            </a>
        </div>
        </div>
                ...
                <div class="col-3 filter-col" id="filter-col">
                    ...
                    <div class="box">
                        ...
                        <ul class="filter-list">
                            ...
                            <li><a href="?isStock=true">In Stock</a></li>
                            <li><a href="?isStock=false">Out of stock</a></li>
                        </ul>
                    </div>
