...
if (isset($_GET['orderid']) && $_GET['orderid'] == 'order') {
    ...
    // Chắc chắn rằng $insertOrder có giá trị và không rỗng trước khi sử dụng nó trong câu lệnh header
    // if (!empty($insertOrder)) {
    //     $order_id_encoded = urlencode($insertOrder); // Mã hóa giá trị $order_id nếu cần thiết

    //     // Chuyển hướng với tham số orderid
    //     header("Location: success.php?orderid=$order_id_encoded");
    //     exit();
    // } else {
    //     // Xử lý khi không có orderid trả về từ insertOrder
