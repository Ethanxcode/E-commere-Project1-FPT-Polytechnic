...
VALUES ('$customer_id', NOW(), '$notes', '$totalPrice', '$address', '$discountAmount',' $quantity')";
    ...
    public function checkCoupon($couponCode, $currentTime, $quantity, $cartId)
    {
        // Update the quantity in the cart
        $updateQuery = "UPDATE tbl_cart SET quantity = '$quantity' WHERE cartId = '$cartId' ";
        $this->db->update($updateQuery);

        if ($couponCode != null && $currentTime != null) {
            // Select the coupon based on the provided code and current time
            $selectQuery = "SELECT * FROM tbl_coupon WHERE couponCode = '$couponCode' AND '$currentTime' >= releaseDate AND '$currentTime' <= expiredDate";
            $couponResult = $this->db->select($selectQuery);

            if ($couponResult) {
                foreach ($couponResult as $coupon) {
    ...
    public function get_cart_order($customer_id)
    {
        ...
        $query = "SELECT * FROM tbl_order_items WHERE customer_id = '$customer_id' ORDER BY order_date DESC LIMIT 1";
