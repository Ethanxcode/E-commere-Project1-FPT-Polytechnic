...
class customer
{
    ...
    public function change_password($id, $data)
    {
        ...
        $oldPassword = md5(htmlspecialchars($data['oldPassword']));
        $newPassword = md5(htmlspecialchars($data['newPassword']));
        $confirmPassword = md5(htmlspecialchars($data['confirmPassword']));
        ...
        $oldPassword = md5($oldPassword); // Mã hóa mật khẩu cũ

        // Kiểm tra mật khẩu cũ có khớp với mật khẩu trong CSDL không
        $query = "SELECT * FROM tbl_customer WHERE id = '$id' AND password = '$oldPassword'";
        $result = $this->db->select($query);

        if ($result) {
            $newPassword = md5($newPassword); // Mã hóa mật khẩu mới

            // Mật khẩu cũ khớp, kiểm tra mật khẩu mới và xác nhận mật khẩu
            if ($newPassword === $confirmPassword) {
                // Thực hiện cập nhật mật khẩu mới
                $query = "UPDATE tbl_customer SET password = '$newPassword' WHERE id = '$id'";
