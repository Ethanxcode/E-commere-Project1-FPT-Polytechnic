...
class customer
{
    ...
    {
        $defaultPassword = md5('password123'); // Mật khẩu mặc định (được mã hóa)
        $defaultFullName = "John Doe"; // Họ tên mặc định
        $defaultPhoneNumber = ""; // Số điện thoại mặc định
        $defaultUsername = ""; // Tên đăng nhập mặc định
        $defaultEmail = ""; // Email mặc định
        ...
        if ($result) {
            // Reset người dùng thành công
            // ...
            return "Reset người dùng thành công.";
        } else {
            return "Reset người dùng không thành công.";
        }
    }
