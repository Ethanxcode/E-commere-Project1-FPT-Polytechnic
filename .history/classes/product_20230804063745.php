...
class product
{
    ...
    {
        $pages = $page * 8 - 8;
        $catName = $this->fm->validation($keyword);
        $query = "SELECT * FROM tbl_product WHERE  productName like '%$keyword%' LIMIT 8 OFFSET $pages ";
        $result = $this->db->select($query);
        return $result;
    }
    ...
    {
        $isStock = boolval($isStock);
        $pages = $page * 8 - 8;

        // Construct the common part of the query
        $query = "SELECT * FROM tbl_product WHERE stock " . ($isStock ? "> 0" : "<= 0") . " LIMIT 8 OFFSET $pages";

        // Execute the query and return the result
        $result = $this->db->select($query);
        return $result;
    }
    public function show_isFeatured_product($isFeatured, $page)
    {

        $pages = $page * 8 - 8;

        // Construct the common part of the query
        $query = "SELECT * FROM tbl_product WHERE type = '$isFeatured' LIMIT 8 OFFSET $pages  ";
