...
class cart
{
    ...
    public function insertOrder($customer_id)
    {
        ...
        $get_product = $this->db->select($query, array($sId));
        ...
        if ($get_product) {
            foreach ($get_product as $result) {
                ...
                VALUES (?, ?, ?, ?, ?, ?)";
                $insert_order = $this->db->insert($insert_order, array($productId, $productName, $quantity, $price, $image, $customer_id));

                if (!$insert_order) {
                    echo "Error occurred while inserting order.";
                    return;
                }
            }
            header('Location: cart.php');
