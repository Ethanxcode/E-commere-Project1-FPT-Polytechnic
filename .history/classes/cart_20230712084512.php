...
class cart
{
    ...
    public function insertCoupon($couponCode, $currentTime)
    {
        $query = "SELECT * FROM tbl_coupon WHERE discountCode = '$couponCode' AND '$currentTime' >= releaseDate AND '$currentTime' <= expiredDate";
        $result = $this->db->select($query);
        if ($result) {

        } else {
            // echo
            // Xử lý tại đây, ví dụ: hiển thị thông báo lỗi cho người dùng
        }
    }
