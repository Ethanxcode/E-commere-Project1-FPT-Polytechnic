...
class cart
{
    ...
    public function get_cart_order($customer_id)
    {
        $query = "SELECT order_id, order_date, product_id, product_name, quantity, price, oi
              FROM tbl_order 
              INNER JOIN tbl_order oi ON o.order_id = oi.order_id
              INNER JOIN tbl_product p ON oi.product_id = p.productId
        ...
        foreach ($get_cart_order as $orderItem) {
            ...
            $productData = array(
                ...
                // Thêm thông tin hình ảnh sản phẩm (nếu cần)
                'status' => $orderItem['status'],
                // Thêm thông tin trạng thái sản phẩm (nếu cần)
                'address' => $orderItem['address'], // Thêm thông tin địa chỉ giao hàng (nếu cần)
            );
