...
class cart
{
    ...
    public function insertOrder($customer_id, $totalPrice, $notes, $address)
    {
        $sId = session_id();
        $query = "SELECT * FROM tbl_cart WHERE sId = '$sId'";
        $get_product = $this->db->select($query);
        ...
        if ($get_product) {
            ...
            $insert_order_items = "INSERT INTO tbl_order_items (customer_id, order_date, notes, total_price, address) 
        VALUES ('$customer_id', NOW(), '$notes', '$totalPrice', '$address')";
            $this->db->insert($insert_order_items);

 $query = "SELECT orderid FROM tbl_order_items ORDER BY order_date DESC LIMIT 1";
$getOrderId = $this->db->select($query);

if ($getOrderId) {
    // Get the order_id from the result array
    $order_id = $getOrderId[0]['orderid'];
    ...
    foreach ($get_product as $result) {
        ...
        $productName = $result['productName'];
            ...
            // Get the last inserted order_id from tbl_order_items
            $order_id = $this->db->select($getOrderId);
            ...
            foreach ($get_product as $result) {
                ...
                // Insert the order items into the tbl_order table, using the retrieved order_id
                $insert_order = "INSERT INTO tbl_order (order_id, productId, productName, quantity, price, image, customer_id) 
            VALUES ('$order_id', '$productId', '$productName', '$quantity', '$price', '$image', '$customer_id')";
                $this->db->insert($insert_order);
            }

            // Clear the cart (tbl_cart) for the current session
            ...
            // If there are no products in the cart, handle it as per your requirements
            echo "Error";
            // header('Location: index.php');
            // exit();
        }
    ...
    {
        $sId = session_id();
        $query = "SELECT * FROM tbl_order_items WHERE customer_id ='$customer_id'";
    ...
    public function get_order_items($customer_id, $order_id)
    {
        $query = "SELECT 
            ...
            FROM
                tbl_order_items oi
            JOIN
                tbl_order o ON oi.order_id = o.id
            WHERE
