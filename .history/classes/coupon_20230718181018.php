...
class coupon
{
    ...
    public function insertCoupon($couponNae)
    {
        $couponNae = $this->fm->validation($couponNae);
        if (empty($couponNae)) {
            $alert = "<span>Không được bỏ trống </span>";
            return $alert;
        } else {
            $query = "INSERT INTO tbl_brand(couponNae) VALUE('$couponNae')";
    ...
    public function UpdateCoupon($couponNae, $id)
    {
        $couponNae = $this->fm->validation($couponNae);

        if (empty($couponNae)) {
            $alert = "<span>Khoogn được bỏ trống couponNae</span>";
            return $alert;
        } else {
            $query = "UPDATE tbl_brand SET couponNae ='$couponNae' WHERE brandId='$id'";
