...
VALUES ('$customer_id', NOW(), '$notes', '$totalPrice', '$address', '$discountAmount')";
    ...
    {
        $query = "SELECT oi.id, oi.productId, oi.orderId, oi.productName, oi.customer_id, SUM(oi.quantity), oi.price, oi.image, oi.status, oi.date_order, c.fullName
              FROM tbl_order_items AS oi
              INNER JOIN tbl_customer AS c ON oi.customer_id = c.id
              
              UNION
              
              SELECT o.id, NULL AS productId, o.orderId, NULL AS productName, o.customer_id, NULL AS quantity, o.total_price AS price, NULL AS image, o.status, o.order_date AS date_order, c.fullName
              FROM tbl_order AS o
