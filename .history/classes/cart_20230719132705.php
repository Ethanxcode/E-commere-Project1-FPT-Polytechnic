...
class cart
{
    ...
    public function insertOrder($customer_id, $totalPrice, $notes, $address)
    {
        ...
        VALUES ('$customer_id', NOW(), '$notes', '$totalPrice', '$address')";
            $order_id = $this->db->insert($insert_order_items);

            if (!$order_id) {
                // If there was an error inserting into tbl_order_items, handle it as per your requirements
                echo "Error inserting into tbl_order_items";
                return;
            }
...
VALUES ('$order_id', '$productId', '$productName', '$quantity', '$price', '$image', '$customer_id')";
                $result = $this->db->insert($insert_order);
            }
