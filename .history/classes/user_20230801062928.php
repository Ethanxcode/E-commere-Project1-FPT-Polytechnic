...
class user
{
    ...
    {
        $query = "SELECT COUNT(*) AS categoryCount FROM tbl_category";
        $result = $this->db->select($query);

        if ($result && isset($result[0]['categoryCount'])) {
            return (int) $result[0]['categoryCount'];
        }
    ...
    {
        $query = "SELECT COUNT(*) AS brandCount FROM tbl_brand";
        $result = $this->db->select($query);

        if ($result && isset($result[0]['brandCount'])) {
            return (int) $result[0]['brandCount'];
        }
