...
class product
{
    ...
    {
        $query = "SELECT tbl_product.*, tbl_category.catName, tbl_brand.brandName, COUNT(tbl_review.comment_id) AS commentCount FROM tbl_product INNER JOIN tbl_category ON tbl_product.catId = tbl_category.catId INNER JOIN tbl_brand ON tbl_product.brandId = tbl_brand.brandId LEFT JOIN tbl_review ON tbl_product.productId = tbl_review.productId GROUP BY tbl_product.productId ORDER BY `commentCount` ASC";
    ...
    public function insert_comment($productid, $customerid, $comment)
    {
        if (strlen($comment) > 220) {
            $alert = "<span>Bình luận phải ngắn hơn 220 ký tự.</span>";
            return $alert;
        }
        ...
        if (empty($comment)) {
            ...
            $query = "INSERT INTO tbl_review(productId,customerId,comment,CreatedAt) VALUE('$productid','$customerid','$comment', '$createdAt')";
            $result = $this->db->insert($query);
            $alert = '
        <div class="alert alert-danger" role="alert">
    ...
    public function productStatisticsAction()
    {
        $query = "SELECT tbl_category.catId, tbl_category.catName,
            COUNT(*) as so_luong,
            MIN(tbl_product.price) as gia_min,
            MAX(tbl_product.price) as gia_max,
            AVG(tbl_product.price) as gia_avg
            FROM tbl_product
            JOIN tbl_category ON tbl_category.catId=tbl_product.catId
            GROUP BY tbl_category.catId, tbl_category.catName";
        $result = $this->db->select($query);
        return $result;
    }
}
