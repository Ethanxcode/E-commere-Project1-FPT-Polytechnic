...
class cart
{
    ...
    public function($discountCode, $currentTime) {
        $query = "SELECT * FROM tbl_coupon WHERE discountCode = '$discountCode' AND '$currentTime' >= releaseDate AND '$currentTime' <= expiredDate";
        $result = $this->db->select($query);
        if($result) {
