    ...
    public function checkCoupon($couponCode, $currentTime)
    {
        ...
        if ($result) {
            foreach ($result as $coupon) {
                ...
                header('Location: offline.php?'.$discountAmount.')
            }
        } else {
            $errorMessage = 'Mã giảm giá không hợp lệ.';
            $response = array(
                'isValid' => false,
                'errorMessage' => '<div class="alert alert-danger" role="alert">' . $errorMessage . ' Đến trang đặt hàng? <a class="btn-danger text-danger" href="/offline.php">Tiếp tục</a></a></div>'
            );
        }
